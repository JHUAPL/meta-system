...
get_taxid_abundance() {
  ...
  grep -A10000000 -P "^%\t" "$1" | tail -n+2 >"$tmp/ku.tmp"

  # get taxid and counts for reads directly assigned to taxids (taxReads)
  awk -F'\t' '{if($3!=0){print($0)}}' "$tmp/ku.tmp" >"$tmp/kr.tmp"
  # discard taxid 0 (unclassifed) assigned reads
  grep -vP "\t0\tno rank\tunclassified$" "$tmp/kr.tmp" >"$tmp/kraken.tmp"

  # calc and output predicted abundance profile
  cut -f3,7 "$tmp/kraken.tmp" | awk -F'\t' '{
		count[$2]+=$1;
		total+=$1;
	}END{
		for(t in count){
...
get_taxid_abundance "$INPUT" >"$OUTDIR/parsed_krakenuniq"
